<template>
  <ion-page>
    <!-- Header reutilizable -->
    <PageHeader profile-trigger-id="provider-profile-trigger" />

    <!-- Menú desplegable del perfil -->
    <ProfilePopover 
      trigger-id="provider-profile-trigger" 
      user-type="provider" 
    />
...
const handleSearch = (query: string, filters: any) => {
  console.log('🔍 Búsqueda desde searchbar en ProviderPage:', { query, filters });
  currentFilters.value = filters;
};

// Función para manejar aplicación de filtros
const handleFiltersApplied = (filters: any) => {
  console.log('Filtros aplicados desde ProviderPage:', filters);
  currentFilters.value = filters;
};
...
const handleLocationSearch = (params: { radius: number; filters: Record<string, any> }) => {
  console.log('🔍 Búsqueda con ubicación iniciada desde ProviderPage:', params);
